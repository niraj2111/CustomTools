...
import { ARTWORK_WIDTH_TO_HEIGHT, BELLY_BUFFER_MM, CONTROLS, PALETTE, PAPER, fromSeed, geometry, normalize, resolve, svg } from './geometry.mjs';
...
function expectedOriginalPath(p, j) {
  ...
  const bulb = format(vaseY + firstAllowedY + (lastAllowedY - firstAllowedY) * p.bulbHeightRatio);
...
function check(input) {
  ...
  assert.equal(p.canvasWMM, PAPER.widthMM);
  assert.equal(p.canvasHMM, PAPER.heightMM);
  ...
  assert.ok(p.a2 >= 0.3 && p.a2 <= 1 && p.a3 >= 0.3 && p.a3 <= 1);
  assert.ok(!(p.a2 < p.bellyWidth && p.a3 < p.bellyWidth));
  ...
  for (const path of drawing.paths) {
    ...
    for (let index = 0; index < numbers.length; index += 2) {
      assert.ok(numbers[index] >= 0 && numbers[index] <= PAPER.widthMM, 'Vase x-coordinate stays on paper');
      assert.ok(numbers[index + 1] >= 0 && numbers[index + 1] <= PAPER.heightMM, 'Vase y-coordinate stays on paper');
    }
  }
  ...
  if (p.showBranch) {
    ...
    for (const line of drawing.branchLines) {
      assert.ok(line.x1 >= 0 && line.x1 <= PAPER.widthMM && line.x2 >= 0 && line.x2 <= PAPER.widthMM);
      assert.ok(line.y1 >= 0 && line.y1 <= PAPER.heightMM && line.y2 >= 0 && line.y2 <= PAPER.heightMM);
    }
  } else {
  ...
  assert.ok(output.includes(`width="${PAPER.widthMM}mm" height="${PAPER.heightMM}mm"`));
...
console.log(`Passed: ${checked} states across all 14 control limits; updated constraints, original path construction, deterministic SVG, and color isolation.`);
